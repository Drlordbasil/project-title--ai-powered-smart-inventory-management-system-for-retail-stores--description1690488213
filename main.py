...
class SmartInventoryManagementSystem:
    ...
    def optimize_inventory(self):
        ...
        reorder_suggestions = self.smart_reordering(forecasted_demand, stock_df)
        
        seasonal_patterns, trends, customer_preferences = self.seasonal_trend_analysis(sales_df)
        product_segments = self.product_segmentation(sales_df)
        
        monitoring_results = self.real_time_monitoring(sales_df, stock_df)
        
        visualizations, reports = self.visualization_reporting(sales_df, stock_df)
